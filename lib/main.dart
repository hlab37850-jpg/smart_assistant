...
class DB {
  ...
  static Future<Database> get d async {
    ...
    return _d = await openDatabase(
      ...
      onCreate: (db, v) async {
        await db.execute(
          'CREATE TABLE customers(id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT NOT NULL,phone TEXT,notes TEXT,created TEXT)',
        );
        await db.execute(
          'CREATE TABLE products(id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT NOT NULL,category TEXT,qty REAL,minimum REAL,price REAL,created TEXT)',
        );
        await db.execute(
          'CREATE TABLE dues(id INTEGER PRIMARY KEY AUTOINCREMENT,customer TEXT,amount REAL,due TEXT,note TEXT,status TEXT)',
        );
        await db.execute(
          'CREATE TABLE transactions(id INTEGER PRIMARY KEY AUTOINCREMENT,customer TEXT,type TEXT,amount REAL,note TEXT,date TEXT)',
        );
      },
    );
  }
...
class _IP extends State<ImportPage> {
  String msg = 'اختر CSV أو XLSX ثم اختر نوع البيانات.';
  bool busy = false;
  ...
  Widget build(BuildContext c) => Scaffold(
        ...
        body: ListView(
          ...
          children: [
            FilledButton.icon(
              onPressed: busy ? null : () => pick(c, 'customers'),
              icon: const Icon(Icons.people),
              label: const Text('استيراد العملاء'),
            ),
            FilledButton.icon(
              onPressed: busy ? null : () => pick(c, 'products'),
              icon: const Icon(Icons.inventory_2),
              label: const Text('استيراد الأصناف'),
            ),
            ...
            const SizedBox(height: 10),
            const Text(
              'صيغة العملاء: name,phone,notes\nصيغة الأصناف: name,category,qty,minimum,price',
              style: TextStyle(color: muted),
            ),
          ],
        ),
  ...
  Future<void> pick(BuildContext c, String type) async {
    setState(() => busy = true);
    try {
      ...
      setState(() => msg = 'تم استيراد $name بنجاح.');
    } catch (e) {
      setState(() => msg = 'فشل الاستيراد: $e');
    } finally {
